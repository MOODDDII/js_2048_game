...
class Game {
    ...
    moveLeft() {
        ...
        for(let row = 0; row < 4; row++){
            ...
            let mergedRow = [];
            let scoreIncrease = 0;
            for(let i = 0; i < newRow.length; i++)if (i < newRow.length - 1 && newRow[i] === newRow[i + 1]) {
    ...
    moveUp() {
        this.field = this.flip();
        this.moveLeft();
        this.field = this.flip(true);
    }
    moveDown() {
        this.field = this.flip();
        this.moveRight();
    ...
    flip(reverse = false) {
        ...
        for(let i = 0; i < 4; i++){
            flipped[i] = [];
            for(let j = 0; j < 4; i++)flipped[i][j] = this.field[j][i];
        }
        reverse ? flipped.map((row)=>row.reverse()) : flipped;
    }
    checkWin() {
...
//# sourceMappingURL=index.4bb9c666.js.map
